...
const CropYieldPrediction = () => {
  ...
  const labels = ['RainFall', 'Fertilizer', 'Temperature', 'Nitrogen', 'Phosphorous', 'Potassium'];
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(`http://localhost:5000/predict2?inputData=${inputDataArray.join(',')}`);
      console.log(response);
      const data = await response.json();

      // Check the prediction result
      if (response.ok) {
        setSubmitMessage(`Predicted crops: ${data.Acres_prediction}`);
  ...
  return (
    <div className="w-screen h-screen bg-gray-50 dark:bg-green-900 flex justify-center items-center">
      <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-4 text-white">Crop Yield Prediction</h1>
